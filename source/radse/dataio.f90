...
SUBROUTINE WRITE_DATA(t, r, field, wavefn, fmsk, tf, phif, &
    E, W, St)
    IMPLICIT NONE
    ...
    varnames(7)  = 'Re(St)';     ! dims(1,:)
    varnames(8)  = 'Im(St)';     ! dims(1,:)
    ...
    DO i=1,nvars
        ...
        CASE (7)
            CALL H5DWRITE_F(dset_id, H5T_REAL_PRECX, REAL(St,num), dims(1,:), error)
        CASE (8)
            CALL H5DWRITE_F(dset_id, H5T_REAL_PRECX, AIMAG(St), dims(1,:), error)
        CASE (9)
