...
SUBROUTINE WRITE_DATA_QCR_2D(t, x, y, VD, field, wavefn, tf, psif, &
    tp, psip1, psip2, Vp, S, St, fmsk, run_ccr)
    IMPLICIT NONE
    REAL(num),    INTENT(IN) :: t(:), x(:), y(:), tf(:), tp(:), Vp(:,:,:), fmsk(:,:)
    COMPLEX(num), INTENT(IN) :: psif(:,:,:), psip1(:,:), psip2(:,:), St(:,:)
    ...
    INTEGER, PARAMETER :: nvars1 = 6, nvars2 = 24
    CHARACTER(LEN=10) :: varnames1(nvars1), varnames2(nvars2)

    INTEGER :: n, i, j, error
    INTEGER, PARAMETER :: rank2 = 2, rank3 = 3
    INTEGER(HSIZE_T) :: dims1(4,2), dims2(15,2), dims_vd(3)
    INTEGER(HSIZE_T) :: dims_psif(3), dims_psip1(2), dims_psip2(2)
    ...
    varnames2(14) = 'Re(St)';       dims2(14,:) = (/SIZE(St,1),SIZE(St,2)/)
    varnames2(15) = 'Im(St)';       dims2(15,:) = (/SIZE(St,1),SIZE(St,2)/)
    varnames2(16) = 'Re(psi)';      dims_psif   = (/nx,ny,ntf/)
    varnames2(17) = 'Im(psi)';    ! dims_psif
    varnames2(18) = 'Vp';         ! dims_psif
    varnames2(19) = 'Re(p-psi1)';   dims_psip1  = (/nx,ntp/)
    varnames2(20) = 'Im(p-psi1)'; ! dims_psip1
    varnames2(21) = 'Re(p-psi2)';   dims_psip2  = (/ny,ntp/)
    varnames2(22) = 'Im(p-psi2)'; ! dims_psip2
    varnames2(23) = 'norm(psi)';  ! dims2(1,:)
    varnames2(24) = 'enrg(psi)';  ! dims2(1,:)
    ...
    DO i=1,nvars2
        ...
        CASE (19:20)
            CALL H5SCREATE_SIMPLE_F(rank2, dims_psip1, dspace_id, error)
        CASE (21:22)
            CALL H5SCREATE_SIMPLE_F(rank2, dims_psip2, dspace_id, error)
        CASE (23:24)
        ...
        CASE (14)
            CALL H5DWRITE_F(dset_id, H5T_REAL_PRECX, REAL(St,num), dims2(i,:), error)
        CASE (15)
            CALL H5DWRITE_F(dset_id, H5T_REAL_PRECX, AIMAG(St), dims2(i,:), error)
        CASE (16)
        ...
        CASE (19)
            CALL H5DWRITE_F(dset_id, H5T_REAL_PRECX, REAL(psip1,num), dims_psip1, error)
        CASE (20)
            CALL H5DWRITE_F(dset_id, H5T_REAL_PRECX, AIMAG(psip1), dims_psip1, error)
        CASE (21)
            CALL H5DWRITE_F(dset_id, H5T_REAL_PRECX, REAL(psip2,num), dims_psip2, error)
        CASE (22)
            CALL H5DWRITE_F(dset_id, H5T_REAL_PRECX, AIMAG(psip2), dims_psip2, error)
        CASE (23)
...
SUBROUTINE WRITE_DATA_QCR_1D(t, x, VD, field, wavefn, tf, psi, Vp, fmsk, run_ccr)
    ...
    varnames2(7)  = 'Re(psi)';      dims_psi = (/nx,ntf/)
...
SUBROUTINE WRITE_INIT_QCR_2D(wavefn)
    IMPLICIT NONE
    TYPE(SchrodingerWavefunction2D), INTENT(IN) :: wavefn
    ...
    varnames = (/'Re(psi)','Im(psi)'/)
    dims = (/SIZE(wavefn%psi,1), SIZE(wavefn%psi,2)/)
    ...
    CALL H5SCREATE_SIMPLE_F(rank2, dims, dspace_id, error)
    CALL H5DCREATE_F(file_id, varnames(1), H5T_REAL_PRECX, dspace_id, dset_id, error)
    CALL H5DWRITE_F(dset_id, H5T_REAL_PRECX, REAL(wavefn%psi,num), dims, error)
    CALL H5DCLOSE_F(dset_id, error)
    CALL H5SCLOSE_F(dspace_id, error)

    CALL H5SCREATE_SIMPLE_F(rank2, dims, dspace_id, error)
    CALL H5DCREATE_F(file_id, varnames(2), H5T_REAL_PRECX, dspace_id, dset_id, error)
    CALL H5DWRITE_F(dset_id, H5T_REAL_PRECX, AIMAG(wavefn%psi), dims, error)
    CALL H5DCLOSE_F(dset_id, error)
    CALL H5SCLOSE_F(dspace_id, error)
    ...
    RETURN
END SUBROUTINE WRITE_INIT_QCR_2D

SUBROUTINE WRITE_INIT_QCR_1D(wavefn)
    IMPLICIT NONE
    TYPE(SchrodingerWavefunction1D), INTENT(IN) :: wavefn
    ...
    ! write initial state data
    CALL H5SCREATE_SIMPLE_F(rank1, dims, dspace_id, error)
    CALL H5DCREATE_F(file_id, varnames(1), H5T_REAL_PRECX, dspace_id, dset_id, error)
    CALL H5DWRITE_F(dset_id, H5T_REAL_PRECX, REAL(wavefn%psi,num), dims, error)
    CALL H5DCLOSE_F(dset_id, error)
    CALL H5SCLOSE_F(dspace_id, error)
    ...
    RETURN
END SUBROUTINE WRITE_INIT_QCR_1D

SUBROUTINE READ_INIT_QCR_2D(wavefn)
    IMPLICIT NONE
    TYPE(SchrodingerWavefunction2D), INTENT(INOUT) :: wavefn
    ...
    CALL H5DGET_SPACE_F(dset_id, dspace_id, error)
    CALL H5SGET_SIMPLE_EXTENT_DIMS_F(dspace_id, dims, dims, error)
    ALLOCATE(Re_psi(dims(1),dims(2)))
    CALL H5DREAD_F(dset_id, H5T_REAL_PRECX, Re_psi, dims, error)

    CALL H5DOPEN_F(file_id, 'Im(psi)', dset_id, error)
    CALL H5DGET_SPACE_F(dset_id, dspace_id, error)
    CALL H5SGET_SIMPLE_EXTENT_DIMS_F(dspace_id, dims, dims, error)
    ALLOCATE(Im_psi(dims(1),dims(2)))
    CALL H5DREAD_F(dset_id, H5T_REAL_PRECX, Im_psi, dims, error)

    ! close file and end HDF5 interface
    CALL H5FCLOSE_F(file_id, error)
    CALL H5CLOSE_F(error)

    wavefn%psi = Re_psi + i*Im_psi
    DEALLOCATE(Re_psi,Im_psi)

    RETURN
END SUBROUTINE READ_INIT_QCR_2D

SUBROUTINE READ_INIT_QCR_1D(wavefn)
    IMPLICIT NONE
    TYPE(SchrodingerWavefunction1D), INTENT(INOUT) :: wavefn
    ...
    CALL H5DGET_SPACE_F(dset_id, dspace_id, error)
    CALL H5SGET_SIMPLE_EXTENT_DIMS_F(dspace_id, dims, dims, error)
    ALLOCATE(Re_psi(dims(1)))
    CALL H5DREAD_F(dset_id, H5T_REAL_PRECX, Re_psi, dims, error)

    CALL H5DOPEN_F(file_id, 'Im(psi)', dset_id, error)
    CALL H5DGET_SPACE_F(dset_id, dspace_id, error)
    CALL H5SGET_SIMPLE_EXTENT_DIMS_F(dspace_id, dims, dims, error)
    ALLOCATE(Im_psi(dims(1)))
    CALL H5DREAD_F(dset_id, H5T_REAL_PRECX, Im_psi, dims, error)
    ...
    RETURN
END SUBROUTINE READ_INIT_QCR_1D
