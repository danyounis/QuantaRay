...
program main
    ...
    ! PARTICLE TRACKING !
    if (enable_tracking) call particle_tracking
    ...
    do it = field%it_on, field%it_off
        ...
        do itp = it, field%it_off, its
            ...
            do ine = ne-N_VD+1, ne
                ...
                select case (mode(1:4))
                case ('2d1e')
                    epdot(1) = -DVsc((/Z1,Z2/),(/electron(ine)%x,electron(ine)%y/),ac(1),1) + Z2*field%Ex(itp)
                    epdot(2) = -DVsc((/Z1,Z2/),(/electron(ine)%x,electron(ine)%y/),ac(1),2) + Z2*field%Ey(itp)
                case ('1d2e')
                    epdot(1) = -DVsc((/Z1,Z2/), (/electron(ine)%x/), ac(1), 1) + Z2*field%Ex(itp) &
                        - DVsc((/Z2,Z2/), (/electron(ine)%x-electron(ine)%y/), ac(3), 1)
                    epdot(2) = -DVsc((/Z1,Z2/), (/electron(ine)%y/), ac(2), 1) + Z2*field%Ex(itp) &
                        - DVsc((/Z2,Z2/), (/electron(ine)%y-electron(ine)%x/), ac(3), 1)
                end select
    ...
    do it = field%it_on, field%it_off
        ...
        do itp = it, field%it_off, its
            ...
            do ine = ne-N_VD+1, ne
                ...
                ! calculate lagrangian
                elagr(1) = (electron(ine)%px**2)/2.0 - Vsc((/Z1,Z2/),(/electron(ine)%x/),ac(1)) &
                ...
                ! recalculate lagrangian
                elagr(2) = (electron(ine)%px**2)/2.0 - Vsc((/Z1,Z2/),(/electron(ine)%x/),ac(1))
...
subroutine particle_tracking
    ...
    if (mode(1:4) == '1d1e') then
        print '(A)', 'Error: Particle-tracking routine not implemented for 1d1e wavefunction.'
...
subroutine calc_coeff_rk4(e, fldt, erkc)
    ...
    erkc(1,3) = dtp*(Z2*fldt(1,1) - DVsc((/Z1,Z2/),(/e%x,e%y/),ac(1),1)) ! K1px
    erkc(1,4) = dtp*(Z2*fldt(2,1) - DVsc((/Z1,Z2/),(/e%x,e%y/),ac(1),2)) ! K1py

    erkc(2,1) = dtp*(e%px + 0.5*erkc(1,3)) ! K2x
    erkc(2,2) = dtp*(e%py + 0.5*erkc(1,4)) ! K2y
    erkc(2,3) = dtp*(Z2*fldt(1,2) - DVsc((/Z1,Z2/),(/e%x+0.5*erkc(1,1),e%y+0.5*erkc(1,2)/),ac(1),1)) ! K2px
    erkc(2,4) = dtp*(Z2*fldt(2,2) - DVsc((/Z1,Z2/),(/e%x+0.5*erkc(1,1),e%y+0.5*erkc(1,2)/),ac(1),2)) ! K2py

    erkc(3,1) = dtp*(e%px + 0.5*erkc(2,3)) ! K3x
    erkc(3,2) = dtp*(e%py + 0.5*erkc(2,4)) ! K3y
    erkc(3,3) = dtp*(Z2*fldt(1,2) - DVsc((/Z1,Z2/),(/e%x+0.5*erkc(2,1),e%y+0.5*erkc(2,2)/),ac(1),1)) ! K3px
    erkc(3,4) = dtp*(Z2*fldt(2,2) - DVsc((/Z1,Z2/),(/e%x+0.5*erkc(2,1),e%y+0.5*erkc(2,2)/),ac(1),2)) ! K3py

    erkc(4,1) = dtp*(e%px + erkc(3,3)) ! K4x
    erkc(4,2) = dtp*(e%py + erkc(3,4)) ! K4y
    erkc(4,3) = dtp*(Z2*fldt(1,3) - DVsc((/Z1,Z2/),(/e%x+erkc(3,1),e%y+erkc(3,2)/),ac(1),1)) ! K4px
    erkc(4,4) = dtp*(Z2*fldt(2,3) - DVsc((/Z1,Z2/),(/e%x+erkc(3,1),e%y+erkc(3,2)/),ac(1),2)) ! K4py

    case ('1d2e')
    erkc(1,1) = dtp*e%px ! K1x1
    erkc(1,2) = dtp*e%py ! K1x2
    erkc(1,3) = dtp*(-DVsc((/Z1,Z2/),(/e%x/),ac(1),1) + Z2*fldt(1,1) & ! K1p1
        - DVsc((/Z2,Z2/),(/e%x-e%y/),ac(3),1))
    erkc(1,4) = dtp*(-DVsc((/Z1,Z2/),(/e%y/),ac(2),1) + Z2*fldt(1,1) & ! K1p2
        - DVsc((/Z2,Z2/),(/e%y-e%x/),ac(3),1))

    erkc(2,1) = dtp*(e%px + 0.5*erkc(1,3)) ! K2x1
    erkc(2,2) = dtp*(e%py + 0.5*erkc(1,4)) ! K2x2
    erkc(2,3) = dtp*(-DVsc((/Z1,Z2/),(/e%x+0.5*erkc(1,1)/),ac(1),1) + Z2*fldt(1,2) &
        - DVsc((/Z2,Z2/),(/e%x+0.5*erkc(1,1)-e%y-0.5*erkc(1,2)/),ac(3),1)) ! K2p1
    erkc(2,4) = dtp*(-DVsc((/Z1,Z2/),(/e%y+0.5*erkc(1,2)/),ac(2),1) + Z2*fldt(1,2) &
        - DVsc((/Z2,Z2/),(/e%y+0.5*erkc(1,2)-e%x-0.5*erkc(1,1)/),ac(3),1)) ! K2p2

    erkc(3,1) = dtp*(e%px + 0.5*erkc(2,3)) ! K3x1
    erkc(3,2) = dtp*(e%py + 0.5*erkc(2,4)) ! K3x2
    erkc(3,3) = dtp*(-DVsc((/Z1,Z2/),(/e%x+0.5*erkc(2,1)/),ac(1),1) + Z2*fldt(1,2) &
        - DVsc((/Z2,Z2/),(/e%x+0.5*erkc(2,1)-e%y-0.5*erkc(2,2)/),ac(3),1)) ! K3p1
    erkc(3,4) = dtp*(-DVsc((/Z1,Z2/),(/e%y+0.5*erkc(2,2)/),ac(2),1) + Z2*fldt(1,2) &
        - DVsc((/Z2,Z2/),(/e%y+0.5*erkc(2,2)-e%x-0.5*erkc(2,1)/),ac(3),1)) ! K3p2

    erkc(4,1) = dtp*(e%px + erkc(3,3)) ! K4x1
    erkc(4,2) = dtp*(e%py + erkc(3,4)) ! K4x2
    erkc(4,3) = dtp*(-DVsc((/Z1,Z2/),(/e%x+erkc(3,1)/),ac(1),1) + Z2*fldt(1,3) &
        - DVsc((/Z2,Z2/),(/e%x+erkc(3,1)-e%y-erkc(3,2)/),ac(3),1)) ! K4p1
    erkc(4,4) = dtp*(-DVsc((/Z1,Z2/),(/e%y+erkc(3,2)/),ac(2),1) + Z2*fldt(1,3) &
        - DVsc((/Z2,Z2/),(/e%y+erkc(3,2)-e%x-erkc(3,1)/),ac(3),1)) ! K4p2
    end select
...
subroutine calc_lagr(e, fldt, elagr)
    ...
    case ('2d1e')
        elagr = 0.5*(e%px**2 + e%py**2) &
            - Vsc((/Z1,Z2/),(/e%x,e%y/),ac(1)) &
            + Z2*e%x*fldt(1) + Z2*e%y*fldt(2)
    case ('1d2e')
        elagr = 0.5*(e%px**2 + e%py**2) &
            - Vsc((/Z1,Z2/), (/e%x/), ac(1)) &
            - Vsc((/Z1,Z2/), (/e%y/), ac(2)) &
            - Vsc((/Z2,Z2/), (/e%x-e%y/), ac(3)) &
            + Z2*(e%x + e%y)*fldt(1)
    end select
...
subroutine remove_bound_trajectories
    ...
    do ine=1,tne
        cnd(1) = 0.5*(electron(ine)%px**2 + electron(ine)%py**2) &
            + Vsc((/Z1,Z2/),(/electron(ine)%x,electron(ine)%y/),ac(1)) < 0.0
    ...
    !$OMP PARALLEL DO SHARED(electron) PRIVATE(cnd)
    do ine=1,tne
        cnd(1) = (electron(ine)%px**2)/2.0 + Vsc((/Z1,Z2/),(/electron(ine)%x/),ac(1)) &
            + Vsc((/Z2,Z2/),(/electron(ine)%x-electron(ine)%y/),ac(3)) < 0.0
        cnd(2) = (electron(ine)%py**2)/2.0 + Vsc((/Z1,Z2/),(/electron(ine)%y/),ac(2)) &
            + Vsc((/Z2,Z2/),(/electron(ine)%y-electron(ine)%x/),ac(3)) < 0.0
...
subroutine ttr_initialize
    ...
    inquire(file='idl.dat', exist=exist(1))
    inquire(file='itl.dat', exist=exist(2))

    if ((.not.exist(1)).and.(.not.exist(2))) then
    ...
    do it=field%it_on,field%it_off
    ...
    else if (exist(1).and.exist(2)) then
        ! count requested no. electrons to track
        open(newunit=iu, file='idl.dat', status='old', action='read')
        do
        ...
        print '(A)', 'Error: Particle tracker needs a data file listing' &
        //' detector- (idl.dat) and time-launch indices (itl.dat).'
        print '(A)', 'Terminating execution.'
        call exit(1)
    end if
...
subroutine ttr_store_data(e, k, it, status)
    ...
    case ('2d1e')
        temp = 0.5*(e%px**2 + e%py**2) &
            + Vsc((/Z1,Z2/),(/e%x,e%y/),ac(1))! - Z2*(e%x*field%Ex(it) + e%y*field%Ey(it))
    ...
    case ('1d2e')
        temp = 0.5*e%px**2 + Vsc((/Z1,Z2/),(/e%x/),ac(1)) &
            + Vsc((/Z2,Z2/),(/e%x-e%y/),ac(3))! - Z2*e%x*field%Ex(it)

        open(k, file='en1-'//trim(adjustl(k_str))//'.dat', status=status, action='write', position='append')
        write(k,*) temp
        close(k)

        temp = 0.5*e%py**2 + Vsc((/Z1,Z2/),(/e%y/),ac(2)) &
            + Vsc((/Z2,Z2/),(/e%y-e%x/),ac(3))! - Z2*e%y*field%Ex(it)
...
subroutine init_runtime_log
    ...
    select case (int_method)
    case ('rk4')
        write(iu,'(A,F4.2,A)') 'Runge-Kutta, dt-push = ',dtp,' au'
    case ('fwe')
        write(iu,'(A,F4.2,A)') 'Forward-Euler, dt-push = ',dtp,' au'
    end select
...
subroutine update_runtime_log
    ...
    't = ', t(it)+dt, ' au    nt: ', it,'  ',1d+2*(it-field%it_on)/(field%it_off-field%it_on),'%'
    close(iu)
    return
