...
interface calc_harmonic_gen
    module procedure harmonic_generation_1DR
end interface calc_harmonic_gen
...
subroutine psi1dr_prepare_atomic_state(this, lp, ntau, dr, dt, pure)
    ...
    if (pure) then
        ...
        do it=1,ntau
            ...
            call tridiag_matmul_cvec(rt, Wp, this%phi(lp+1,:))

            ! solve (W-).phi(t+dt) = rt
            call tridiag_fbwd_subs(Wm, this%phi(lp+1,:), rt)
        ...
        do l=0,l_max
            ...
            do it=1,ntau
                ! calculate rt = (W+).phi(t)
                call tridiag_matmul_cvec(rt, Wp, this%phi(l+1,:))

                ! solve (W-).phi(t+dt) = rt
                call tridiag_fbwd_subs(Wm, this%phi(l+1,:), rt)
...
real(num) function expectE_2D_cnn(this, V, dr)
    ...
    if (.not.allocated(ltx)) then
        ...
        allocate(rtx(nr(1),nr(2)), rty(nr(1),nr(2)))
    end if
    ...
    expectE_2D_cnn = -0.5*dr(1)*dr(2)*trapz(real(ltx*rtx,num)) &
        - 0.5*dr(1)*dr(2)*trapz(real(lty*rty,num)) &
        + dr(1)*dr(2)*trapz(real(transpose(conjg(this%psi))*V*this%psi,num))
...
subroutine harmonic_generation_1DR(this, Et, dr, S)
    implicit none
    class(SchrodingerWavefunction1DR), intent(in) :: this
    ...
    return
end subroutine harmonic_generation_1DR
...
subroutine tSURFF2D_dt_step(S, wavefn, A, C, x, y, t)
    ...
    real(num) :: r(2), k(2), r_hat(2), r_mag, k_para, k_sq
    ...
    do iky=1,S%nk(2)
        ...
        do j=1,S%Ns
            ...
            volkovc = ((2.*pi)**(-3/2.))*exp(i*k_sq*t/2.)*exp(-i*sum(k*(r-C)));
