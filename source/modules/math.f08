...
function fliplr_rm(M,ax) result(f_M)
    ...
    real(num), allocatable :: f_M(:,:)
    integer :: nn(2), j1, j2

    nn(1) = size(M,1); nn(2) = size(M,2);
...
function inverse(M) result(C)
    implicit none
    real(num), intent(in) :: M(:,:)
    real(num), allocatable :: C(:,:)
    integer :: n

    real(num), allocatable :: L(:,:), U(:,:), a(:,:), b(:), d(:), x(:)
    real(num) :: coeff
    integer :: i, j, k

    n = size(M,1)
    allocate(C(n,n), L(n,n), U(n,n), a(n,n), b(n), d(n), x(n))

    a = M; L = 0.0; U = 0.0; b = 0.0;
...
subroutine fft_shift_2d(func)
    ...
    nn(1) = size(func,1); nn(2) = size(func,2);
...
function fft_1d(func) result(ft_func)
    ...
    complex(num), allocatable :: ft_func(:)
    real(num),    allocatable :: ft_temp(:)

    integer :: n
    n = size(func)

    allocate(ft_func(n), ft_temp(2*n))
...
function fft_2d(func) result(ft_func)
    ...
    allocate(ft_func(nn(1),nn(2)), ft_temp(2*nn(1)*nn(2)))
...
function ifft_1d(func) result(ift_func)
    ...
    complex(num), allocatable :: ift_func(:)
    real(num),    allocatable :: ift_temp(:)

    integer :: n
    n = size(func)

    allocate(ift_func(n), ift_temp(2*n))
...
function ifft_2d(func) result(ift_func)
    ...
    allocate(ift_func(nn(1),nn(2)), ift_temp(2*nn(1)*nn(2)))
...
function grad_r1d(f,del) result(gradf)
    ...
    real(num), allocatable :: gradf(:)
    integer :: n, j

    n = size(f)
    allocate(gradf(n))

    gradf(1) = -3.0*f(1) + 4.0*f(2) - f(3)
    do j=2,n-1
        gradf(j) = f(j+1) - f(j-1)
    end do
    gradf(n) = 3.0*f(n) - 4.0*f(n-1) + f(n-2)
...
function grad_r2d(f,del,ax) result(gradf)
    ...
    nn(1) = size(f,1); nn(2) = size(f,2);
    allocate(gradf(nn(1),nn(2)))
