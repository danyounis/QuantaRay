...
subroutine emf_trapezoidal_pulse(this, t)
    ...
    this%Ex = this%E0/sqrt(1.0+this%eps**2) * ft * sin(carrier)
    this%Ey = this%E0/sqrt(1.0+this%eps**2) * ft * this%eps*cos(carrier)
...
subroutine emf_sine_squared_pulse(this, t)
    ...
    if ((t(it) >= this%t_on).and.(t(it) <= this%t_off)) then
        ...
        gt(it) = sin(pi*(t(it)-this%t_on)/this%Tp) * cos(pi*(t(it)-this%t_on)/this%Tp)
    end if
    end do
    ...
    this%Ax = -this%E0/this%omg0/sqrt(1.0+this%eps**2) * ft * sin(carrier)
    this%Ay = -this%E0/this%omg0/sqrt(1.0+this%eps**2) * ft * this%eps*cos(carrier)

    ! construct electric field
    this%Ex = this%E0/this%omg0/sqrt(1.0+this%eps**2) &
        * ((2*pi/this%Tp)*gt*sin(carrier) + (this%omg0 + 2*this%ch1*(t-this%t_on))*ft*cos(carrier))
    this%Ey = this%E0/this%omg0/sqrt(1.0+this%eps**2) * this%eps &
        * ((2*pi/this%Tp)*gt*cos(carrier) - (this%omg0 + 2*this%ch1*(t-this%t_on))*ft*sin(carrier))
