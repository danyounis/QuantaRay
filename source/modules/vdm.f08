...
subroutine vdet_initialize(this, geom, R0, Nv, x, y, n, nt, sdim)
    implicit none
    class(vdet), intent(inout) :: this

    character(len=6), intent(in) :: geom
    real(num), intent(in) :: x(:), y(:)
    real(num), intent(in) :: R0
    ...
    case (2)
        ...
        case ('square')
            dr = 2.*R0/real(Nv/4-1,num)
            if (n <= Nv/4) then
                this%xl = R0
                this%yl = -R0 + real(n-1,num)*dr
            else if ((n > Nv/4).and.(n <= Nv/2)) then
                this%xl = R0 - real(n-Nv/4-1,num)*dr
                this%yl = R0
            else if ((n > Nv/2).and.(n <= 3*Nv/4)) then
                this%xl = -R0
                this%yl = R0 - real(n-Nv/2-1,num)*dr
            else
                this%xl = -R0 + real(n-3*Nv/4-1,num)*dr
                this%yl = -R0
            end if
        end select
